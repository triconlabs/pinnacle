    ...
    actions: {
        ...
        runCloudCode: function(number) {
            ...
            this.get('cloud').run('hello', {
                ...
                d3.layout.cloud().size([width, height]).words(preqlist).rotate(function(d) {
                        return 0;
                    })
                    //   .font("Impact")
                    .fontSize(function(d) {
                ...
                function draw(words) {
                    ...
                    Ember.$("svg").remove();
                    d3.select(".answers").append("svg").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")").selectAll("text").data(words).enter().append("text").style("font-size", function(d) {
                            return d.size + "px";
                        })
